...
const token = __ENV.ORDERS_AUTH_TOKEN || __ENV.AUTH_TOKEN;
...
if (!token) {
  fail('ORDERS_AUTH_TOKEN or AUTH_TOKEN is required; pass a valid Orders-audience bearer token.');
}

setResponseCallback(expectedStatuses({ min: 200, max: 399 }, 409));
...
export default function () {
  const idempotencyKey = `orders-payments-cb-${__VU}-${__ITER}-${Date.now()}-${Math.floor(Math.random() * 1000000)}`;
  const payload = JSON.stringify({
    idempotency_key: idempotencyKey,
    user_id: `orders-payments-cb-user-${__VU}`,
    user_email: `orders-payments-cb-user-${__VU}@eurotransit.local`,
    train_id: trainId,
  ...
  const response = http.post(`${baseUrl}/api/v1/orders`, payload, {
    ...
    tags: {
      ...
      test: 'orders-payments-circuit-breaker',
    },
  });
  ...
  const expected = response.status === 202 || response.status === 409;
  unexpectedStatus.add(!expected);

  check(response, {
    'order create returned 202 or 409': () => expected,
