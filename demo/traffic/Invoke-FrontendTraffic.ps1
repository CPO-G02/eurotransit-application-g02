...
Invoke-EuroTransitTraffic -Service frontend -Destination $destination -Path '/' `
    -DurationMinutes $settings.DurationMinutes -DurationSeconds $settings.DurationSeconds `
    -RequestsPerMinute $settings.RequestsPerMinute -TimeoutSeconds $settings.TimeoutSeconds `
    -MaxConcurrency $settings.MaxConcurrency `
    -MinimumRequestVolumePercentage $settings.MinimumRequestVolumePercentage `
    -TrafficMode 'application-read' `
